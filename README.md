...
| Video `Demo/` | Nặng, không cần cho build | (lưu riêng) |

**Quy ước ngôn ngữ trong repo:**
- **Mã nguồn**: comment / docstring / log đã được chuyển sang **tiếng Anh**.
- **README** (mọi cấp): giữ **tiếng Việt**.
- **Cố ý giữ tiếng Việt** (không phải sót): prompt của VLM trong `layer2_brain/config.json` (robot suy luận & trả lời tiếng Việt — đúng mục tiêu KLTN), nhãn giao diện app MAUI, nhãn các hình Chương 4 (`scripts/plot_*.py`, `tools/plot_*.py`) và khoá JSON số liệu benchmark (`docs/bang_4_*.json`).

`yolov8n.pt` (~6 MB) **được giữ lại** trong `layer1_vision/model/` để chạy thị giác offline ngay.
...
|---|---|---|---|
| **A. Pipeline VLM (3 lớp)** | `layer1_vision/` · `layer2_brain/` · `layer3_control/` | Nguyên mẫu "bộ não" + bộ đo hiệu năng Chương 4: Camera → YOLO → VLM → quyết định → điều khiển tay gắp | Windows / Linux (dev), Jetson |
...
- **Stack A** trả lời câu hỏi *"vật này có thu gom được không?"* — dùng để benchmark YOLO‑only vs VLM (Chương 4).
...
```
┌──────────────┐   ảnh + YOLO    ┌──────────────┐    gọi VLM    ┌──────────────┐
│  Layer 1     │ ─────────────▶  │  Layer 2     │ ───────────▶  │  llama.cpp   │
│  Vision Node │                 │  Brain Server│               │  Qwen2.5-VL  │
│  (client)    │ ◀── JSON ─────  │  (FastAPI)   │ ◀──────────── │  :8080       │
└──────┬───────┘                 └──────────────┘               └──────────────┘
       │ collectible=true → POST /execute
       ▼
┌──────────────┐
│  Layer 3     │ ──▶  Kinematics → tay gắp (Serial/PWM)
│  Control Node│
│  (Flask :8001)│
└──────────────┘
```
...
| 3 | `layer3_control/control_node.py` | Nhận lệnh → kinematics → điều khiển tay | 8001 | [layer3_control/README.md](layer3_control/README.md) |

## B. Robot tự hành ROS2 (`ros2_ws/`)

```
Astra depth ─▶ /scan ─▶ slam_toolbox ─▶ /map ─┐
                                              ├─▶ NAV2 ─▶ /cmd_vel ─▶ wifi_node ─▶ (WiFi) ─▶ ĐẾ ESP32
explorer (WFD) ─▶ frontier goal ─────────────┘                          ▲
YOLO (color) ─▶ /yolo/detections ─▶ orchestrator (state machine) ───────┘
ĐẾ: EKF ─▶ ROBOT_STATE ─▶ (WiFi) ─▶ wifi_node ─▶ /odom (nuôi lại SLAM + NAV2)
```

7 package ROS2 — chi tiết & lệnh build/run: **[ros2_ws/README.md](ros2_ws/README.md)** · vận hành: [docs/DEMO_RUNBOOK.md](docs/DEMO_RUNBOOK.md) · bàn giao: [docs/HANDOFF.md](docs/HANDOFF.md).
...
├── layer1_vision/      # Stack A · Camera + YOLO + Astra depth + trigger        → README riêng
├── layer2_brain/       # Stack A · FastAPI + Qwen2.5-VL qua llama.cpp           → README riêng
├── layer3_control/     # Stack A · Flask + kinematics tay gắp (placeholder)     → README riêng
│
├── ros2_ws/            # Stack B · workspace ROS2 Foxy (7 package)              → README riêng
├── Source_code/        # Firmware đế/tay + cầu CAN + app MAUI                   → README riêng
│
├── scripts/            # Benchmark, eval accuracy, verify_setup, start VLM      → README riêng
├── tools/              # Công cụ Astra (calib/noise/stream) + setup AP WiFi     → README riêng
├── config/             # astra_intrinsics.json (nội tham số camera)
├── data/               # eval_dataset (ảnh + labels.json) — minh chứng Chương 4
